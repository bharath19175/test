...
![Architecture Diagram](https://user-images.githubusercontent.com/12695796/29464587-39885f84-842e-11e7-9a92-b84eaf7559eb.png)
...
1. Packetbeat is configured to use the hosts network, in order to capture traffic on the host system rather than that between the containers.  However, for Windows and OSX based systems this "host system" represents the linux VM, hosting docker, running on Hyper-V (windows) or HyperKit(OSX). The native interface on the actual host system is currently not visible to Packetbeat for these operating systems.  The user will therefore only see traffic between the containers and host VM. Improvements and contributions welcome in this area. 
